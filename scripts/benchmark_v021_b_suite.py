...
def main():
    print("================================================================================")
    print("STARTING V021-B INDUSTRIAL LIVESTOCK BENCHMARK SUITE (FRESH SEEDS)")
    print("================================================================================")
    
    # 12 FRESH deterministic seeds (400-411)
    FRESH_SEEDS = [400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411]
    
    # Load candidate agents
    v020_c_fn = load_agent("agents/v020_c_competitive_surgical.py")
    v021_b_fn = load_agent("agents/v021_b_industrial_livestock.py")
    ...
    print("\n--- 1. DIRECT HEAD-TO-HEAD: V021-B vs V020-C ---")
    h2h_res = run_matchup(v021_b_fn, v020_c_fn, "V021-B", "V020-C", FRESH_SEEDS)
    print(f"Result: Win Rate: {h2h_res['win_rate_a']:.1f}% ({h2h_res['wins_a']}W / {h2h_res['wins_b']}L / {h2h_res['ties']}T)")
    print(f"V021-B Mean Bank: ${h2h_res['mean_a']:,.0f} | V020-C Mean Bank: ${h2h_res['mean_b']:,.0f} | Delta: ${h2h_res['delta']:+,.0f} | Min: ${h2h_res['min_a']:,.0f}")
    ...
    for opp_name, opp_fn in opponents.items():
        ...
        res_c = run_matchup(v020_c_fn, opp_fn, "V020-C", opp_name, FRESH_SEEDS)
        res_b = run_matchup(v021_b_fn, opp_fn, "V021-B", opp_name, FRESH_SEEDS)
        ...
        print(f"  V021-B vs {opp_name:25s} | WR: {res_b['win_rate_a']:5.1f}% ({res_b['wins_a']}W/{res_b['wins_b']}L) | Bank: ${res_b['mean_a']:6,.0f} vs ${res_b['mean_b']:6,.0f} | Min: ${res_b['min_a']:6,.0f}")
    ...
    with open("experiments/v021_b_benchmark_results.json", "w") as f:
        json.dump({"h2h": h2h_res, "field": all_summary}, f, indent=2)
        
    print("\n================================================================================")
    print("BENCHMARK COMPLETE. Results saved to experiments/v021_b_benchmark_results.json")
    print("================================================================================")
