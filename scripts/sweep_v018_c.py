...
print("===============================")
print("V018-C Threshold Sweep (T=10, 15, 20, 25, 30)")
print("===============================")
    ...
    for seed in seeds:
        ...
        if os.path.exists(metrics_path):
            with open(metrics_path) as mf:
                ...
                rejections.append(m.get("c1_rejections", 0))
                plants.append(sum(c["planted"] for c in m.get("crops", {}).values()))
                
    print(f"T={t}: Mean Bank=${np.mean(banks):.2f} (Plants: {np.mean(plants):.1f}, Rejections: {np.mean(rejections):.1f})")
