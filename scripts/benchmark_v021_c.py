...
def main():
    print("================================================================================")
    print("STARTING V021-C TWO-STAGE PIVOT BENCHMARK SUITE (12 FRESH SEEDS: 500-511)")
    print("================================================================================")
    
    FRESH_SEEDS = [500, 501, 502, 503, 504, 505, 506, 507, 508, 509, 510, 511]
    
    v020_c_fn = load_agent("agents/v020_c_competitive_surgical.py")
    v021_c_fn = load_agent("agents/v021_c_two_stage_pivot.py")
    ...
    print("\n--- 1. DIRECT HEAD-TO-HEAD: V021-C vs V020-C ---")
    h2h_res = run_matchup(v021_c_fn, v020_c_fn, "V021-C", "V020-C", FRESH_SEEDS)
    print(f"Result: Win Rate: {h2h_res['win_rate_a']:.1f}% ({h2h_res['wins_a']}W / {h2h_res['wins_b']}L / {h2h_res['ties']}T)")
    print(f"V021-C Mean Bank: ${h2h_res['mean_a']:,.0f} | V020-C Mean Bank: ${h2h_res['mean_b']:,.0f} | Delta: ${h2h_res['delta']:+,.0f} | Min: ${h2h_res['min_a']:,.0f}")
    ...
    for opp_name, opp_fn in opponents.items():
        ...
        res_cand = run_matchup(v021_c_fn, opp_fn, "V021-C", opp_name, FRESH_SEEDS)
        ...
        print(f"  V021-C vs {opp_name:25s} | WR: {res_cand['win_rate_a']:5.1f}% ({res_cand['wins_a']}W/{res_cand['wins_b']}L) | Bank: ${res_cand['mean_a']:6,.0f} vs ${res_cand['mean_b']:6,.0f} | Min: ${res_cand['min_a']:6,.0f}")
    ...
    with open("experiments/v021_c_benchmark_results.json", "w") as f:
        json.dump({"h2h": h2h_res, "field": all_summary}, f, indent=2)
        
    print("\n================================================================================")
    print("BENCHMARK COMPLETE. Results saved to experiments/v021_c_benchmark_results.json")
    print("================================================================================")
