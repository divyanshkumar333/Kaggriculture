...
def main():
    print("================================================================================")
    print("STARTING V021-D THROUGHPUT SCALING BENCHMARK SUITE")
    print("================================================================================")
    
    H2H_SEEDS = list(range(700, 720)) # 20 Fresh seeds
    FIELD_SEEDS = list(range(720, 732)) # 12 Fresh seeds
    
    v020_c_fn = load_agent("agents/v020_c_competitive_surgical.py")
    v021_d_fn = load_agent("agents/v021_d_throughput_scaling.py")
    ...
    opponents = {
        ...
        "Dynamic Clusters (V008-D)": load_agent("agents/v008_d_dynamic_clusters.py"),
        "Melon Flooder": load_agent("agents/opp_melon_flooder.py"),
        "Starter": "starter",
        "Random": "random"
    }
    
    print(f"\n--- 1. DIRECT HEAD-TO-HEAD: V021-D vs V020-C ({len(H2H_SEEDS)} FRESH SEEDS: 700-719) ---")
    h2h_res = run_matchup(v021_d_fn, v020_c_fn, "V021-D", "V020-C", H2H_SEEDS)
    print(f"Result: Win Rate: {h2h_res['win_rate_a']:.1f}% ({h2h_res['wins_a']}W / {h2h_res['wins_b']}L / {h2h_res['ties']}T)")
    print(f"V021-D Mean Bank: ${h2h_res['mean_a']:,.0f} | V020-C Mean Bank: ${h2h_res['mean_b']:,.0f} | Delta: ${h2h_res['delta']:+,.0f} | Min: ${h2h_res['min_a']:,.0f}")
    
    print(f"\n--- 2. FULL BENCHMARK SUITE ACROSS ALL OPPONENTS ({len(FIELD_SEEDS)} SEEDS: 720-731) ---")
    all_summary = []
    ...
    for opp_name, opp_fn in opponents.items():
        ...
        res_c = run_matchup(v020_c_fn, opp_fn, "V020-C", opp_name, FIELD_SEEDS)
        res_d = run_matchup(v021_d_fn, opp_fn, "V021-D", opp_name, FIELD_SEEDS)
        
        print(f"  V020-C vs {opp_name:25s} | WR: {res_c['win_rate_a']:5.1f}% ({res_c['wins_a']}W/{res_c['wins_b']}L) | Bank: ${res_c['mean_a']:6,.0f} vs ${res_c['mean_b']:6,.0f} | Min: ${res_c['min_a']:6,.0f}")
        print(f"  V021-D vs {opp_name:25s} | WR: {res_d['win_rate_a']:5.1f}% ({res_d['wins_a']}W/{res_d['wins_b']}L) | Bank: ${res_d['mean_a']:6,.0f} vs ${res_d['mean_b']:6,.0f} | Min: ${res_d['min_a']:6,.0f}")
    ...
    with open("experiments/v021_d_benchmark_results.json", "w") as f:
        json.dump({"h2h": h2h_res, "field": all_summary}, f, indent=2)
        
    print("\n================================================================================")
    print("BENCHMARK COMPLETE. Results saved to experiments/v021_d_benchmark_results.json")
    print("================================================================================")
