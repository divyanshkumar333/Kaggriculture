"""
Efficient Frontier Counterfactual Sweep for Kaggriculture V025
==============================================================
Evaluates:
1. Cow Cap Sweep: [8, 9, 10, 11, 12, 13]
2. Strawberry Cap Sweep: [30, 36, 42, 48, 54]
3. Cow Start Day Sweep: [3, 4, 5]
4. Cow vs Strawberry priority trade-offs

Measures against V023-G Champion on paired seeds (P0 and P1).
"""
...
def run_single_eval(args):
    label, cow_cap, straw_cap, cow_start_day, straw_start_day, seeds = args
    agent_fn = make_agent(cow_cap, straw_cap, cow_start_day, straw_start_day)
    ...
    for s in seeds:
        ...
        scores.append(r0)
        opp_scores.append(opp_r0)
        deltas.append(r0 - opp_r0)
        if r0 > opp_r0: wins += 1
        ...
        scores.append(r1)
        opp_scores.append(opp_r1)
        deltas.append(r1 - opp_r1)
        if r1 > opp_r1: wins += 1
    ...
    return {
        ...
        "straw_start_day": straw_start_day,
        "mean_bank": float(np.mean(scores)),
        "median_bank": float(np.median(scores)),
        "opp_mean": float(np.mean(opp_scores)),
        ...
        "min_bank": float(np.min(scores)),
        "max_bank": float(np.max(scores))
    }

def main():
    seeds = list(range(6000, 6035)) # 35 seeds = 70 paired games per config
    configs = [
        # Baseline V025-A
        ("V025-A Baseline (11 cows, 42 straw, D4)", 11, 42, 4, 9, seeds),
        
        # 1. Cow Cap Sweep
        ("Cow Cap = 8", 8, 42, 4, 9, seeds),
        ("Cow Cap = 9", 9, 42, 4, 9, seeds),
        ("Cow Cap = 10", 10, 42, 4, 9, seeds),
        ("Cow Cap = 11 (Standard)", 11, 42, 4, 9, seeds),
        ("Cow Cap = 12", 12, 42, 4, 9, seeds),
        ("Cow Cap = 13", 13, 42, 4, 9, seeds),

        # 2. Strawberry Cap Sweep
        ("Straw Cap = 30", 11, 30, 4, 9, seeds),
        ("Straw Cap = 36", 11, 36, 4, 9, seeds),
        ("Straw Cap = 42 (Standard)", 11, 42, 4, 9, seeds),
        ("Straw Cap = 48", 11, 48, 4, 9, seeds),
        ("Straw Cap = 54", 11, 54, 4, 9, seeds),

        # 3. Cow Start Day Sweep
        ("Cow Start Day = 3", 11, 42, 3, 9, seeds),
        ("Cow Start Day = 4 (Standard)", 11, 42, 4, 9, seeds),
        ("Cow Start Day = 5", 11, 42, 5, 9, seeds),

        # 4. Strawberry Start Day Sweep
        ("Straw Start Day = 7", 11, 42, 4, 7, seeds),
        ("Straw Start Day = 8", 11, 42, 4, 8, seeds),
        ("Straw Start Day = 9 (Standard)", 11, 42, 4, 9, seeds),
        ("Straw Start Day = 10", 11, 42, 4, 10, seeds),

        # 5. Optimal Combined Candidate
        ("Opt Combined: 11 cows, 42 straw, D4 cow, D8 straw", 11, 42, 4, 8, seeds),
        ("Opt Aggressive: 12 cows, 42 straw, D4 cow, D8 straw", 12, 42, 4, 8, seeds),
    ]

    pool_size = min(8, mp.cpu_count())
    print("="*110)
    print(f"RUNNING EFFICIENT FRONTIER SWEEP OVER {len(configs)} CONFIGURATIONS (70 Games per Config)")
    print("="*110)

    results = []
    with mp.Pool(processes=pool_size) as pool:
        for res in pool.imap(run_single_eval, configs):
            print(f"[{res['experiment']:<48}] -> Mean: ${res['mean_bank']:7,.0f} | Med: ${res['median_bank']:7,.0f} | Delta: +${res['mean_delta']:6,.0f} | WR: {res['win_rate']:5.1f}%", flush=True)
            results.append(res)
    ...
    print("\n" + "="*110)
    print("EFFICIENT FRONTIER SUMMARY TABLE:")
    print("="*110)
    print(df[["experiment", "mean_bank", "median_bank", "mean_delta", "win_rate", "p10", "p25", "min_bank", "max_bank"]].to_string(index=False))
