"""
Production Gate Benchmark Suite for V025-A and V025-F
=====================================================
Evaluates:
1. Efficient frontier parameter grid
2. Head-to-Head tournament between V025-F and V025-A
"""
...
def load_agent(agent_name):
    if agent_name == "v25f":
        spec = importlib.util.spec_from_file_location("mod_f", "agents/v025_f_adaptive_flywheel.py")
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        return mod.agent
    elif agent_name == "v25a":
        spec = importlib.util.spec_from_file_location("mod_a", "agents/v025_a_aggressive_cows.py")
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        return mod.agent
    elif agent_name == "v23g":
        spec = importlib.util.spec_from_file_location("mod_g", "agents/v023_g_capital_optimizer.py")
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        return mod.agent
    elif agent_name.startswith("param_"):
        # Format: param_c{cow_cap}_s{straw_cap}_cd{c_day}_sd{s_day}
        parts = agent_name.split("_")
        cow_cap = int(parts[1][1:])
        straw_cap = int(parts[2][1:])
        cow_start_day = int(parts[3][2:])
        straw_start_day = int(parts[4][2:])
        return make_param_agent(cow_cap, straw_cap, cow_start_day, straw_start_day)
    else:
        raise ValueError(f"Unknown agent name: {agent_name}")

def make_param_agent(cow_cap, straw_cap, cow_start_day, straw_start_day):
...
# Worker Function
# -----------------------------------------------------------------------------
def execute_game(args):
    seed, name_p0, name_p1 = args
    agent0 = load_agent(name_p0)
    agent1 = load_agent(name_p1)
...
def main():
    print("="*100)
    print("STARTING PRODUCTION-GATE BENCHMARK SUITE")
    print("="*100)

    # -------------------------------------------------------------------------
    # PART 1: Efficient Frontier Parameter Sweep
    # -------------------------------------------------------------------------
    frontier_configs = [
        ("V025-A Baseline", "param_c11_s42_cd4_sd9"),
        ("Cow Cap = 8",     "param_c8_s42_cd4_sd9"),
        ("Cow Cap = 9",     "param_c9_s42_cd4_sd9"),
        ("Cow Cap = 10",    "param_c10_s42_cd4_sd9"),
        ("Cow Cap = 12",    "param_c12_s42_cd4_sd9"),
        ("Cow Cap = 13",    "param_c13_s42_cd4_sd9"),
        ("Straw Cap = 30",  "param_c11_s30_cd4_sd9"),
        ("Straw Cap = 48",  "param_c11_s48_cd4_sd9"),
        ("Cow Start D3",    "param_c11_s42_cd3_sd9"),
        ("Cow Start D5",    "param_c11_s42_cd5_sd9"),
    ]

    p1_seeds = [6000, 6001, 6002, 6003, 6004] # 5 paired seeds = 10 games per config
    p1_tasks = []
    for label, agent_code in frontier_configs:
        for s in p1_seeds:
            p1_tasks.append((s, agent_code, "v23g")) # P0
            p1_tasks.append((s, "v23g", agent_code)) # P1

    print(f"\n--- PART 1: Efficient Frontier Grid ({len(p1_tasks)} games) ---")
    p1_data = {code: {"scores": [], "opp_scores": [], "wins": 0, "total": 0} for _, code in frontier_configs}

    with concurrent.futures.ProcessPoolExecutor(max_workers=3) as executor:
        for res in executor.map(execute_game, p1_tasks):
            p0, p1 = res["name_p0"], res["name_p1"]
            r0, r1 = res["r0"], res["r1"]
            if p0 != "v23g":
                cand_code = p0
                cand_r, opp_r = r0, r1
            ...
            p1_data[cand_code]["scores"].append(cand_r)
            p1_data[cand_code]["opp_scores"].append(opp_r)
            if cand_r > opp_r:
                p1_data[cand_code]["wins"] += 1
            p1_data[cand_code]["total"] += 1

    frontier_rows = []
    print("\n" + "="*85)
    print("EFFICIENT FRONTIER RESULTS:")
    print("="*85)
    for label, code in frontier_configs:
        d = p1_data[code]
        m = float(np.mean(d["scores"]))
        med = float(np.median(d["scores"]))
        opp_m = float(np.mean(d["opp_scores"]))
        delta = m - opp_m
        ...
        print(f"[{label:<20}] -> Mean: ${m:7,.0f} | Med: ${med:7,.0f} | Delta: +${delta:6,.0f} | WR: {wr:5.1f}%", flush=True)
    ...
    # -------------------------------------------------------------------------
    p2_seeds = list(range(7000, 7025)) # 25 paired seeds = 50 games
    p2_tasks = []
    for s in p2_seeds:
        p2_tasks.append((s, "v25f", "v25a")) # P0
        p2_tasks.append((s, "v25a", "v25f")) # P1

    print(f"\n--- PART 2: TOURNAMENT V025-F VS V025-A ({len(p2_tasks)} games) ---")
    f_scores = []
    a_scores = []
    deltas = []
    f_wins = 0
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=3) as executor:
        for res in executor.map(execute_game, p2_tasks):
            if res["name_p0"] == "v25f":
                rf, ra = res["r0"], res["r1"]
            else:
                ra, rf = res["r0"], res["r1"]
    ...
    print("\n" + "="*100)
    print("TOURNAMENT COMPLETE: V025-F VS V025-A")
    print("="*100)
    ...
    print(f"V025-F Win Rate: {stats_tourney['v025_f']['win_rate']:.2f}% ({f_wins}/{tot_games})")
    print("="*100)
