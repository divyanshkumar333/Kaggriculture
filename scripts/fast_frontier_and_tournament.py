"""
Fast Frontier Evaluation & 300-Seed Tournament for V025-F vs V025-A
===================================================================
Runs:
Part 1: Counterfactual parameter grid (Cows 8..13, Strawberries 30..48, Opening velocity)
Part 2: 300-seed paired tournament between V025-F and V025-A (600 games)
"""
...
# -----------------------------------------------------------------------------
# Single Paired Simulation Unit
# -----------------------------------------------------------------------------
def eval_paired_match(args):
    agent_spec_tuple, opp_type, seed = args
    # agent_spec_tuple: ("V025-F",) or ("V025-A",) or ("PARAM", cow_cap, straw_cap, cow_start_day, straw_start_day)
...
def main():
    ...
    print("="*100)
    print(f"STARTING COMBINED FAST AUDIT (FRONTIER + 300-SEED TOURNAMENT) ON {pool_size} CORES")
    print("="*100)

    # -------------------------------------------------------------------------
    # PART 1: Counterfactual Parameter Frontier
    # -------------------------------------------------------------------------
    frontier_configs = [
        ...
        ("Cow Cap = 11",    ("PARAM", 11, 42, 4, 9)),
        ("Cow Cap = 12",    ("PARAM", 12, 42, 4, 9)),
        ("Cow Cap = 13",    ("PARAM", 13, 42, 4, 9)),
        ("Straw Cap = 30",  ("PARAM", 11, 30, 4, 9)),
        ("Straw Cap = 36",  ("PARAM", 11, 36, 4, 9)),
        ("Straw Cap = 42",  ("PARAM", 11, 42, 4, 9)),
        ("Straw Cap = 48",  ("PARAM", 11, 48, 4, 9)),
        ("Cow Start D3",    ("PARAM", 11, 42, 3, 9)),
        ("Cow Start D4",    ("PARAM", 11, 42, 4, 9)),
        ("Cow Start D5",    ("PARAM", 11, 42, 5, 9)),
        ("Straw Start D8",  ("PARAM", 11, 42, 4, 8)),
        ("Straw Start D9",  ("PARAM", 11, 42, 4, 9)),
    ]

    p1_seeds = list(range(6000, 6020)) # 20 seeds = 40 games per config
    p1_tasks = []
    for label, spec in frontier_configs:
        for s in p1_seeds:
            p1_tasks.append((spec, "V023-G", s))

    print(f"Running Part 1: Parameter Frontier ({len(p1_tasks)} match tasks)...", flush=True)
    p1_results = {}
    with mp.Pool(processes=pool_size) as pool:
        for res in pool.imap_unordered(eval_paired_match, p1_tasks):
            spec = res["agent_spec"]
    ...
    for label, spec in frontier_configs:
        ...
        print(f"[{label:<22}] -> Mean: ${mean_s:7,.0f} | Med: ${med_s:7,.0f} | Delta: +${delta_s:6,.0f} | WR: {wr:5.1f}%", flush=True)
    ...
    # -------------------------------------------------------------------------
    # PART 2: 300-Seed Tournament (V025-F vs V025-A)
    # -------------------------------------------------------------------------
    p2_seeds = list(range(7000, 7300)) # 300 seeds = 600 games
    p2_tasks = [(("V025-F",), "V025-A", s) for s in p2_seeds]
    print("\n" + "="*100)
    print(f"Running Part 2: 300-Seed Tournament (V025-F vs V025-A: {len(p2_tasks)} match tasks = 600 games)...", flush=True)
    ...
    with mp.Pool(processes=pool_size) as pool:
        for i, res in enumerate(pool.imap_unordered(eval_paired_match, p2_tasks), 1):
            ...
            if i % 50 == 0 or i == len(p2_tasks):
                ...
                print(f"Tournament: {i}/{len(p2_tasks)} seeds ({p2_results['total']} games) | V025-F WR: {cur_wr:.1f}% | Mean F: ${cur_mean_f:,.0f} | Mean A: ${cur_mean_a:,.0f} | Delta: +${cur_mean_f - cur_mean_a:,.0f}", flush=True)
