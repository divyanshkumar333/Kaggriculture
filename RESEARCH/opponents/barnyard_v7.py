...
def _terminal_liquidation(obs, action, step):
    ...
    market = [order for order in market if len(order) > 0 and order[0] not in ["BUY_SEED", "BUY_ANIMAL", "BUY_PRODUCT", "BUY_LAND"]]
    for item, qty in shed.items():
        if qty > 0 and not any(len(o) >= 3 and o[0] == "SELL" and o[1] == item for o in market):
            if len(market) < 10:
                market.append(["SELL", str(item), max(0, int(qty))])
    action["market"] = market[:10]
    return action
