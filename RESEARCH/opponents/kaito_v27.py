...
def _future_target(step, item, state):
    # Kaito v27 Strict-Future: NO PEEKING AT _ACTIONS
...
def _meta_reset(obs, action, step):
    if step < 360:
        return action
    market_inv = _get(_get(obs, "market", {}), "inventory", {})
    milk_inv = int(market_inv.get("MILK", 10000))
    action = _copy_action(action)
    if milk_inv > 11000:
        market = []
        for order in action.get("market") or []:
            if len(order) >= 3 and order[0] in ["BUY_ANIMAL", "BUY_PRODUCT"] and order[1] in ["COW", "MILK", "WHEAT"]:
                continue
            market.append(order)
...
def agent(obs, configuration=None):
    try:
        ...
        action = _meta_reset(obs, action, step)
