...
class DailyPlanner:
    ...
    def plan_tasks(self):
        ...
        # --- V018-F: Crop Evaluation with Subsistence Fallback ---
        crop_actions = {
        ...
        # V018-F: eval_crops evaluates a list of crops and returns (best_crop, best_profit_per_action).
        # sunk_labor=True: treat worker cost as 0 (idle workers are already paid for — filler-crop mode).
        def eval_crops(crop_list, sunk_labor=False):
            best_p = -999999
            best_c = self.strategy.config.crop_policy
            for crop_name in crop_list:
                ...
                expected_revenue = sum(
                    self.econ.get_price_at_inventory(crop_name, current_inv + i)
                    for i in range(achievable_yield)
                )

                # ponytail: sunk_labor skips historical worker cost for filler crops.
                # Ceiling: underestimates true labor cost of filler; upgrade if labor cost
                # becomes dominant (e.g. late-season with many hands).
                expected_worker_cost = 0 if sunk_labor else crop_actions[crop_name] * historical_cost_per_action
                ...
                expected_net_profit = expected_revenue - seed_cost - expected_worker_cost - expected_crop_loss
                profit_per_action = expected_net_profit / crop_actions[crop_name]

                if profit_per_action > best_p:
                    best_p = profit_per_action
                    best_c = crop_name
            return best_c, best_p

        # Step 1: evaluate premium crops with full labor cost
        best_crop, best_profit_per_action = eval_crops(["TOMATO", "STRAWBERRY", "MELON"])

        # Step 2: V018-F Subsistence Fallback — if no premium crop is profitable and
        # we have idle land, fill it with Wheat/Carrot using sunk-labor EV.
        V018F_FALLBACK_TRIGGER = int(os.environ.get("V018_F_FALLBACK_TRIGGER", 10))
        if best_profit_per_action <= 0 and len(empty_tiles) > V018F_FALLBACK_TRIGGER:
            fb_crop, fb_profit = eval_crops(["WHEAT", "CARROT"], sunk_labor=True)
            if fb_profit > 0:
                best_crop = fb_crop
                best_profit_per_action = fb_profit
                metrics["v018"]["staple_crop_quantity"] += 0  # marker for debugging

        # Step 3: also allow staples to compete alongside premium on normal eval
        # (V018-B evaluated all crops together; re-run to avoid regressing that benefit)
        premium_best_profit = best_profit_per_action
        all_crop_best, all_crop_profit = eval_crops(["WHEAT", "CARROT", "TOMATO", "STRAWBERRY", "MELON"])
        if all_crop_profit > premium_best_profit:
            best_crop = all_crop_best
            best_profit_per_action = all_crop_profit
