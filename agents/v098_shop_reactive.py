...
def agent(obs):
    ...
    if day == 0 and hour == 0:
        ...
        # -------------------------------------------------------------
        # FIRST-SHOP REACTIVITY (EXPERIMENT 8)
        # -------------------------------------------------------------
        town = obs.get("town", {})
        shops = town.get("unlocked_shops", [])
        if day == 3 and hour == 1:
            if "BAKERY" in shops:
                # Buy extra wheat seeds and stop cow rush slightly early
                market_orders.append(["BUY_SEED", "WHEAT", 10])
            elif "ICE_CREAM" in shops:
                # Early strawberry pivot
                market_orders.append(["BUY_SEED", "STRAWBERRY", 15])
            elif "PIZZA" in shops:
                # Tomato demand
                market_orders.append(["BUY_SEED", "TOMATO", 10])
