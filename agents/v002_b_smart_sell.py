    ...
    def plan_tasks(self):
        ...
        for product, qty in self.state.shed.items():
            if qty > 0:
                ...
                # Let's find how many we can sell before marginal price drops
                current_inv = self.state.market.get("inventory", {}).get(product, 10000)
                
                base_price = CROPS.get(product, {}).get("base", 50)
                if product not in CROPS:
                    # fallback for animals/fertilizer if they were hardcoded
                    base_price = MARKET_PARAMS.get(product, {}).get("base", 50)
                
                # Stop selling if marginal price drops below a reasonable threshold of base
