            ...
            for i, (ux, uy) in enumerate(units):
                for j, target in enumerate(field_tasks):
                    ...
                    if target.priority >= 1000:
                        # Urgent: massive negative ensures assignment to SOME worker
                        cost = -1000000 + dist * 10
                    else:
                        cost = (dist * 10) - target.priority

                    # V013-B: Soft Spatial Affinity ONLY (+80 cross-quadrant penalty)
                    # No stability bonus — avoids stale-commitment cascade seen in V012-B.
                    u_quad = (ux // 5, uy // 5)
                    t_quad = (tx // 5, ty // 5)
                    if u_quad != t_quad:
                        cost += 80
