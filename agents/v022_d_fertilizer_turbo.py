"""
Agent V022-D: Industrial Meta Architecture with Fertilizer Turbocharged Strawberries
-------------------------------------------------------------------------------------
Building upon V022-C's 99.5% tournament win rate:
1. Fertilizer Application on Strawberries:
   - Fertilizing ongoing strawberries DOUBLES daily output from 1 to 2 units/day!
   - 40 mature strawberries produce 80 strawberries/day -> ~$12,000-$16,000/day.
   - Internal 15-animal livestock engine continuously produces 15 fertilizer/day.
2. Multi-Hour Paced Market Pipeline:
   - Hour 0: Top-priority workforce hiring (up to 12 hands) + urgent sales.
   - Hour 1: Capital deployment (Quad purchases, Cows, Strawberry seeds, feed buffer).
   - Hours 2-23: Paced selling matching town demand sinks.
3. Quad-Matched Labor Allocation:
   - Scales from 2 workers (Days 0-5) to 12 workers (Days 14-30).
4. Compact Pasture Sector (Rows 0..4, Cols 2..7):
   - 1-3 step transit for 100% daily animal feeding, caring, and milking.
"""
...
WEIGHT_URGENT_WATER = 1900   # Prevent crop death / weed conversion
WEIGHT_FEED_ANIMAL = 1500    # Prevent animal starvation
WEIGHT_CARE_ANIMAL = 1400    # Compounding production bonus
WEIGHT_FERTILIZE_STRAW = 1200 # Double strawberry production
...
def agent(obs):
    ...
    main_farmer_pos = tuple(me["farmer"])
    hands_pos = [tuple(h) for h in me["hands"]]
    all_units = [main_farmer_pos] + hands_pos
    num_units = len(all_units)
    
    # -------------------------------------------------------------------------
    # 1. BOARD SCAN & TELEMETRY
    # -------------------------------------------------------------------------
    animals_on_board = []
    empty_structures = []
    plants_on_board = []
        ...
        for c in range(10):
            ...
            if t == "LOCKED":
                ...
                if kind == "WEED":
                    ...
                    is_fertilized = fert_until >= day
                    plants_on_board.append((c, r, crop, wat, yu, age, max_life, unwat, is_fertilized))
                    if crop == "STRAWBERRY": num_strawberries += 1
                    elif crop == "WHEAT": num_wheat_plants += 1

    num_animals = len(animals_on_board)
    num_plants = len(plants_on_board)

    # -------------------------------------------------------------------------
    # 2. PACED MULTI-HOUR MARKET PIPELINE & TOWN DEMAND EXPLOITATION
    # -------------------------------------------------------------------------
    market_orders = []
    
    # Day 0 Special Scripted Opening
    if day == 0 and hour == 0:
        ...
        # HOUR 0: LABOR HIRING FIRST & URGENT SALES
        if hour == 0:
            if day < 6:
                target_hands = 2
            elif num_quads >= 4:
                ...
                target_hands = 10 if money >= 200 else 6
            elif num_quads >= 2:
            ...
            # In endgame (day >= 26) or surplus fertilizer, sell excess fertilizer
            fert_in_shed = shed.get("FERTILIZER", 0)
            if (day >= 26 or fert_in_shed > 15) and len(market_orders) < 10:
                market_orders.append(["SELL", "FERTILIZER", min(fert_in_shed, 10)])

        # HOUR 1: CAPITAL DEPLOYMENT (Land, Animals, Seeds, Feed)
        elif hour == 1:
            # 1. Animal Feed Replenishment (Guaranteed 3-day buffer)
            ...
            elif "SW" not in unlocked_quads and money >= 2800 and day >= 10:
                market_orders.append(["BUY_LAND"])
            elif "SE" not in unlocked_quads and money >= 7000 and day >= 14:
                market_orders.append(["BUY_LAND"])
                
            # 3. Livestock Expansion
            ...
            # 4. Large-Scale Strawberry & Wheat Seeds
        ...
        # HOURS 2-23: PACED DYNAMIC SELLING (Exploiting Town Demand Sinks)
        else:
    ...
    # A) Animal Maintenance Tasks (High Priority)
    ...
    # B) Plant Tasks (Urgent Water, Regular Water, Fertilizer, Harvest)
    for (px, py, crop, is_wat, y_u, age, max_life, unwat, is_fert) in plants_on_board:
        is_ripe = False
        ...
        if is_ripe and y_u > 0:
            ...
            w_val = WEIGHT_URGENT_WATER if unwat >= 1 else WEIGHT_WATER_PLANT
            tasks.append({"type": "WATER", "pos": (px, py), "weight": w_val, "crop": crop})
        elif crop == "STRAWBERRY" and not is_fert and day <= 25 and (shed.get("FERTILIZER", 0) > 0 or any(inv.get("FERTILIZER", 0) > 0 for inv in inventories if isinstance(inv, dict))):
            tasks.append({"type": "FERTILIZE", "pos": (px, py), "weight": WEIGHT_FERTILIZE_STRAW, "crop": crop})
    ...
    for (sx, sy, skind) in empty_structures:
        target_animal = "COW" if skind == "PASTURE" and (shed.get("COW", 0) > 0 or any(inv.get("COW", 0) > 0 for inv in inventories if isinstance(inv, dict))) else ("SHEEP" if shed.get("SHEEP", 0) > 0 else None)
        if target_animal:
            tasks.append({"type": "PLACE_ANIMAL", "pos": (sx, sy), "weight": WEIGHT_PLACE_ANIMAL, "animal": target_animal})
            
    # E) Build Pastures in Compact Sector
    ...
    if current_pastures < target_pasture_count and money > 300 and empty_unlocked_tiles:
        ...
        for pt in candidate_tiles[:3]:
            tasks.append({"type": "BUILD_PASTURE", "pos": pt, "weight": WEIGHT_BUILD_STRUCTURE})
            
    # F) Plant Seeds (Strawberries & Wheat in open fields)
    ...
    if (avail_straw > 0 or avail_wheat > 0 or avail_melon > 0) and empty_unlocked_tiles:
        ...
        for it in interior_tiles[:25]:
            chosen_crop = "WHEAT"
            if avail_melon > 0 and day < 5: chosen_crop = "MELON"
            elif avail_straw > 0 and day >= 8: chosen_crop = "STRAWBERRY"
    ...
    # Day 0 Scripted Opening for Turn 0..23
    if day == 0:
    ...
    # Days 1-30: Dynamic Hungarian Task Assignment with Role Affinity
    if not unit_actions:
        ...
        for u_idx, upos in enumerate(all_units):
            ...
            if upos in SHED_TILES:
                # 1. Drop harvested products
                holding_prods = sum(v for k, v in inv.items() if k not in ["WHEAT", "SHEEP", "COW", "GOOSE", "FERTILIZER"])
                if holding_prods > 0:
                    pre_actions[u_idx] = ["DROP"]
                    continue
                # 2. Pick up animal if structures are empty
                if empty_structures and (shed.get("COW", 0) > 0 or shed.get("SHEEP", 0) > 0) and not (inv.get("COW") or inv.get("SHEEP")):
                    ...
                    continue
                # 3. Dedicated livestock keepers (u_idx <= 2) maintain wheat
                if num_animals > 0 and inv_wheat < 3 and shed.get("WHEAT", 0) > 0 and u_idx in [0, 1]:
                    wheat_to_pick = min(4, shed.get("WHEAT", 0))
                    pre_actions[u_idx] = ["PICKUP", "WHEAT", wheat_to_pick]
                    continue
                # 4. Field workers (u_idx >= 2) pick up fertilizer if available
                if u_idx >= 2 and inv.get("FERTILIZER", 0) < 2 and shed.get("FERTILIZER", 0) > 0 and day <= 25:
                    fert_to_pick = min(3, shed.get("FERTILIZER", 0))
                    pre_actions[u_idx] = ["PICKUP", "FERTILIZER", fert_to_pick]
                    continue
        ...
        if not tasks:
            ...
            for u_idx, upos in enumerate(all_units):
                ...
                for t in tasks:
                    ...
                    # Dedicated Livestock Keepers (Units 0, 1) prefer animal tasks
                    if u_idx in [0, 1] and t["type"] in ["FEED", "CARE", "HARVEST_ANIMAL", "COLLECT_FERTILIZER", "BUILD_PASTURE", "PLACE_ANIMAL"]:
                        score += 300
                    # Field Workers (Units 2+) prefer crops/fertilizer/weeds
                    elif u_idx >= 2 and t["type"] in ["WATER", "FERTILIZE", "HARVEST_CROP", "PLANT", "DIG"]:
                        score += 200
                        
                    # Penalize FEED if unit has NO wheat
                    if t["type"] == "FEED" and inv_wheat == 0:
                        score -= 3000
                    # Penalize FERTILIZE if unit has NO fertilizer
                    if t["type"] == "FERTILIZE" and inv_fert == 0:
                        score -= 3000
                    # Penalize PLACE_ANIMAL if unit has NO animal
                    if t["type"] == "PLACE_ANIMAL" and inv.get(t.get("animal", ""), 0) == 0:
                        score -= 3000
                    # Penalize tasks if inventory nearly full
                    if inv_count >= 8 and t["type"] not in ["FEED", "CARE", "FERTILIZE"]:
                        score -= 500
                    row.append(-score)
                ...
                if u_idx in assignment:
                    ...
                    if upos == t_pos:
                        if t_type == "FEED":
                            ...
                            elif upos in SHED_TILES and shed.get("FERTILIZER", 0) > 0:
                                unit_actions.append(["PICKUP", "FERTILIZER", 3])
                            else:
                                closest_shed = min(SHED_TILES, key=lambda s: manhattan(upos, s))
                                unit_actions.append([get_move_toward(upos, closest_shed)])
