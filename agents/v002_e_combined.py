...
class StrategyConfig:
    def __init__(self,
                 crop_policy="MELON",
                 min_sell_price_ratio=0.5, # Stop selling if price falls below 50% of base
                 sell_batch_size=10,
        ...
        self.min_sell_price_ratio = min_sell_price_ratio
...
class DailyPlanner:
    ...
    def plan_tasks(self):
        ...
        for y in range(self.state.board_size):
            for x in range(self.state.board_size):
                ...
                if isinstance(tile, dict):
                    ...
                    # Pick the best seed we have available
        ...
        for product, qty in self.state.shed.items():
            if qty > 0:
                current_inv = self.state.market.get("inventory", {}).get(product, 10000)
                base_price = CROPS.get(product, {}).get("base", 50)
                if product not in CROPS:
                    base_price = MARKET_PARAMS.get(product, {}).get("base", 50)
                
                threshold = max(2, base_price * self.strategy.config.min_sell_price_ratio)
        ...
        for crop_name in crop_actions.keys():
            ...
            for y in range(self.state.board_size):
                for x in range(self.state.board_size):
                    tile = farm_tiles[y][x]
                    if isinstance(tile, dict) and tile.get("kind") == "PLANT" and tile.get("crop") == crop_name:
                        planted_count += 1
                        
            pipeline = self.state.shed.get(crop_name, 0) + (planted_count + self.state.seeds.get(crop_name, 0)) * crop_max_yield[crop_name]
            
            current_inv = self.state.market.get("inventory", {}).get(crop_name, 10000)
            sim_inv = current_inv + pipeline
            expected_revenue = 0
            ...
            profit_per_action = expected_profit / crop_actions[crop_name]
            
            if profit_per_action > best_profit:
                best_profit = profit_per_action
                best_crop = crop_name
        ...
        if self.state.seeds.get(target_crop, 0) == 0 and self.state.money > self.strategy.config.cash_reserve + CROPS[target_crop]["seed"]:
            # Production Throttling Logic (don't plant if future price is too low)
            current_inv = self.state.market.get("inventory", {}).get(target_crop, 10000)
            planted_count = 0
            farm_tiles = self.state.my_farm.get("tiles", [])
            for y in range(self.state.board_size):
                for x in range(self.state.board_size):
                    ...
                    if isinstance(tile, dict) and tile.get("kind") == "PLANT" and tile.get("crop") == target_crop:
                        planted_count += 1
            pipeline_qty = (planted_count + self.state.seeds.get(target_crop, 0)) * crop_max_yield[target_crop]
            pipeline_qty += self.state.shed.get(target_crop, 0)
            future_price = self.econ.get_price_at_inventory(target_crop, current_inv + pipeline_qty)
            
            base_price = CROPS.get(target_crop, {}).get("base", 50)
            if best_profit > 1.0 and future_price > base_price * 0.3:
