...
class DailyPlanner:
    ...
    def plan_tasks(self):
        ...
        for crop_name in crop_actions.keys():
            ...
            # V017-C: Evaluate net expected profit by directly taxing EV with estimated dynamic labor cost.
            # A plant takes ~1 action to plant, ~1 to harvest, and ~2 to water (conservative baseline: 4 actions).
            estimated_actions_per_plant = 4
            labor_cost_per_plant = estimated_actions_per_plant * historical_cost_per_action
            
            # 5. Net Expected Profit
            expected_net_profit = expected_revenue - seed_cost - labor_cost_per_plant
            
            # If the crop literally loses money on margins, ban it
            if expected_net_profit <= 0:
                expected_net_profit = -9999
