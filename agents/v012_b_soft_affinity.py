...
class ActionExecutor:
    ...
    def execute(self, tasks, assignments):
        ...
        if field_tasks and units:
            ...
            # Module-level global to persist across steps for this game seed
            import collections
            import os
            if not hasattr(self, "_PREV_ASSIGNMENTS"):
                self._PREV_ASSIGNMENTS = collections.defaultdict(dict)
            seed = os.environ.get("KAGGRICULTURE_SEED", "unknown")
            ...
            for i, (ux, uy) in enumerate(units):
                for j, target in enumerate(field_tasks):
                    ...
                    # 1. Soft Spatial Affinity (+80 penalty for cross-quadrant)
                    u_quad = (ux // 5, uy // 5)
                    t_quad = (tx // 5, ty // 5)
                    if u_quad != t_quad:
                        cost += 80
                        
                    # 2. Anti-Thrashing Stability Bonus (-40 cost to continue existing task)
                    prev_target_loc = self._PREV_ASSIGNMENTS[seed].get(i)
            ...
            for i, j in zip(row_ind, col_ind):
                ...
                self._PREV_ASSIGNMENTS[seed][i] = field_tasks[j].location
