...
class DailyPlanner:
    ...
    def plan_tasks(self):
        ...
        for crop_name in crop_actions.keys():
            ...
            # V016-B: Future Supply Discounting. Add the entire pipeline to the current inventory
            # to project the price when this new seed will actually be harvested.
            projected_inv = self.state.market.get("inventory", {}).get(crop_name, 10000) + pipeline_qty
            expected_revenue = 0
            for i in range(achievable_yield):
                p = self.econ.get_price_at_inventory(crop_name, projected_inv + i)
                expected_revenue += p
