...
class DailyPlanner:
    ...
    def plan_tasks(self):
        ...
        for crop_name in crop_actions.keys():
            ...
            # --- V018-E Pipeline Pacing ---
            TARGET_PIPELINE = float(os.environ.get("V018_E_TARGET_PIPELINE", 100.0))
            ...
            if pipeline > TARGET_PIPELINE:
                if "e_rejections" not in metrics:
                    metrics["e_rejections"] = 0
                metrics["e_rejections"] += 1
                continue # Crop rejected by inventory pacing constraint
