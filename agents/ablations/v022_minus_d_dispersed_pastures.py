"""
Ablation D: V022-C minus Compact Zoning (Dispersed Pastures)
"""
import math
...
def agent(obs):
    ...
    if day == 0 and hour == 0:
        ...
        if hour == 0:
            target_hands = 2 if day < 6 else (12 if num_quads >= 4 else (10 if num_quads >= 3 else 6))
    ...
    # ABLATION: Pastures placed on ANY empty unlocked tile (no compact cluster)
    ...
    if current_pastures < target_pasture_count and money > 300 and empty_unlocked_tiles:
        for pt in empty_unlocked_tiles[:3]: tasks.append({"type": "BUILD_PASTURE", "pos": pt, "weight": WEIGHT_BUILD_STRUCTURE})
    ...
    if (avail_straw > 0 or avail_wheat > 0 or avail_melon > 0) and empty_unlocked_tiles:
        interior = [pt for pt in empty_unlocked_tiles if pt not in SHED_TILES]
