...
class ActionExecutor:
    ...
    def _get_quadrant_name(self, x, y):
    ...
    def execute(self, tasks, assignments):
        ...
        if field_tasks and units:
            ...
            for i, (ux, uy) in enumerate(units):
                worker_quad_name = unlocked_quads[i % len(unlocked_quads)]
                
                for j, target in enumerate(field_tasks):
                    tx, ty = target.location
                    target_quad_name = self._get_quadrant_name(tx, ty)
                    
                    dist = abs(ux - tx) + abs(uy - ty)
                    
                    if target_quad_name != worker_quad_name:
                        cost = 1000000
                    else:
