"""
Candidate V025-C: Replay-Derived Target Envelope Controller
------------------------------------------------------------
Features:
1. Dynamic tracking of the proven top-replay asset trajectory:
   - D4:  1 Cow,  3 Strawberries, 4 Workers
   - D5:  1 Cow,  7 Strawberries, 7 Workers
   - D6:  2 Cows, 12 Strawberries, Q2 Unlock, 7 Workers
   - D8:  8 Cows, 21 Strawberries, 8 Workers
   - D10: 11 Cows, 33 Strawberries, Q3 Unlock, 13 Workers
2. Proactive pasture pre-construction to eliminate placement latency.
3. Liquidity-driven market execution on every 6-hour interval.
"""
...
TARGET_ENVELOPE = {
    0: {"cows": 0, "straw": 0,  "hands": 2, "quads": 1},
    1: {"cows": 0, "straw": 1,  "hands": 3, "quads": 1},
    2: {"cows": 0, "straw": 2,  "hands": 3, "quads": 1},
    3: {"cows": 0, "straw": 3,  "hands": 3, "quads": 1},
    4: {"cows": 1, "straw": 3,  "hands": 3, "quads": 1},
    5: {"cows": 1, "straw": 7,  "hands": 6, "quads": 1},
    6: {"cows": 2, "straw": 12, "hands": 6, "quads": 2},
    7: {"cows": 5, "straw": 18, "hands": 6, "quads": 2},
    8: {"cows": 8, "straw": 21, "hands": 7, "quads": 2},
    9: {"cows": 10, "straw": 28, "hands": 9, "quads": 2},
    10: {"cows": 11, "straw": 33, "hands": 12, "quads": 3},
}
...
def agent(obs):
    ...
    # -------------------------------------------------------------------------
    env_target = TARGET_ENVELOPE.get(day, {"cows": 11, "straw": 42, "hands": 12, "quads": 3})
    ...
    if day == 0 and hour == 0:
        ...
        if hour in [1, 6, 12, 18]:
            ...
            # Envelope-Targeted Cow Acceleration
            cows_in_shed = shed.get("COW", 0)
            target_cows = env_target["cows"]
            if (num_cows + cows_in_shed) < target_cows:
                while money >= 1500 and (num_cows + cows_in_shed) < target_cows and len(market_orders) < 8:
                    market_orders.append(["BUY_ANIMAL", "COW", 1])
                    money -= 1500
                    cows_in_shed += 1
            elif day > 10 and (num_cows + cows_in_shed) < 11 and money >= 2000:
                market_orders.append(["BUY_ANIMAL", "COW", 1])
                money -= 1500
                
            # Envelope-Targeted Strawberry Expansion
            target_straw = env_target["straw"]
            straw_seeds = seeds.get("STRAWBERRY", 0)
            if (num_strawberries + straw_seeds) < target_straw and money >= 100 and len(empty_unlocked_tiles) > 1:
                buy_straw = min(8, target_straw - (num_strawberries + straw_seeds))
