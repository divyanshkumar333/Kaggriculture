"""
Agent V026-C: Pure Imitation Policy (Empirical Distribution Prior)
------------------------------------------------------------------
Samples macro decisions directly from the empirical action distributions
mined from the Top 1% of the 12,430 imitation learning games:
1. Opening Archetype: Exact empirical modal opening (2 Cows, 2 Sheep, 11 Melons, 5 Hires).
2. Daily Macro Schedule: Follows the empirical milestone curve (Cow ramp D4-D12, Strawberry D6-D18).
3. Hard Safety Wrapper: Strict zero-starve, zero-weed Hungarian assignment.
"""
...
def agent(obs):
    ...
    # 1. PURE IMITATION MACRO CONTROLLER
    if day == 0 and hour == 0:
        ...
        if hour == 0:
            # Empirical Median Labor Schedule
            target_hands = 5 if day < 3 else (6 if day < 6 else (10 if num_quads >= 2 else 5))
            if num_quads >= 3 and day >= 10: target_hands = 12
        ...
        if hour in [1, 6, 12, 18]:
            ...
            if day >= 6 and day <= 20 and money >= 250:
                if (num_strawberries + straw_seeds) < 38 and len(empty_unlocked_tiles) > 3:
                    buy_straw = min(10, 38 - (num_strawberries + straw_seeds))
