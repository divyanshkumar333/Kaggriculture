...
def _hybrid_v27_market(action, obs, step):
    ...
    # 2. Calibrated Cow Capacity (Optimal: 8 Cows)
    private = _get(obs, "private", {})
    shed = _get(private, "shed", {})
    cows_in_shed = _get(shed, "COW", 0)
    
    num_cows = 0
    for row in _get(me, "tiles", []):
        for tile in row:
            if isinstance(tile, dict) and tile.get("kind") in ["COOP", "PASTURE"] and tile.get("animal") == "COW":
                num_cows += 1
                
    total_cows = num_cows + cows_in_shed
    ...
    # 3. Delayed Strawberry Powerhouse (Day 8-9 start, 40 capacity)
    num_strawberries = 0
    for row in _get(me, "tiles", []):
        for tile in row:
    ...
    straw_seeds = _get(seeds, "STRAWBERRY", 0)
    
    final_market = []
    for order in new_market:
        if len(order) >= 3 and order[0] == "BUY_SEED" and order[1] == "STRAWBERRY":
            if day < 8:
                continue
            if (num_strawberries + straw_seeds) >= 40:
                continue
            straw_seeds += order[2]
        final_market.append(order)
