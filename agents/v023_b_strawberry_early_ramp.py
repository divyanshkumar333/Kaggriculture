"""
Agent V022-C: Multi-Product Market Batching & Town Sink Exploitation
--------------------------------------------------------------------
Evolution from V022-B:
1. Town Sink & Paced Dynamic Pricing Engine:
   - Detects market inventory and current prices.
   - Avoids single-turn market dumping that crashes sale prices to $1.
   - Paces high-margin sales (Milk, Wool, Strawberries, Melons) across hours 0..22 to ride town shop demand drains ($180-$290 price retention).
2. Priority Order Batching:
   - Hour 0: High-priority workforce hiring + urgent sales.
   - Hour 1: Capital deployment (Quad purchases, Cows, Strawberry seeds, feed).
   - Hours 2-23: Continuous paced selling matching town consumption.
3. Dedicated Role Partitioning:
   - Livestock Keepers (Units 0, 1): Stay in Manhattan <= 3 radius of shed for 100% daily feed/care/harvest/fertilizer.
   - Field Workers (Units 2+): Focus on large-scale strawberry planting, urgent watering, and crop harvesting.
4. Internal Self-Sustaining Feed Engine:
   - Dedicated wheat rotation supplies 100% of animal feed without market dependency.
"""
...
WEIGHT_URGENT_WATER = 1800   # Prevent crop death / weed conversion
WEIGHT_FEED_ANIMAL = 1400    # Prevent animal starvation
WEIGHT_CARE_ANIMAL = 1300    # Compounding production bonus
...
def agent(obs):
    ...
    if day == 0 and hour == 0:
        ...
        if hour == 0:
            ...
            # Sell fertilizer (constant base price $100)
        ...
        # HOURS 2-23: PACED DYNAMIC SELLING (Exploiting Town Demand Sinks)
            ...
            # High-margin items sold in batches of 4-8 to prevent market crash
            for prod in ["MILK", "STRAWBERRY", "WOOL", "MELON", "FERTILIZER", "CARROT", "EGG"]:
                if len(market_orders) >= 10: break
                p_count = shed.get(prod, 0)
                if p_count > 0:
                    # If price is high (>= base), sell up to 8 units
                    # If price dropped, sell in small drip batches of 4 units to allow town recovery
                    cur_price = market_prices.get(prod, 100)
                    batch_size = 8 if cur_price >= 80 else 4
                    if day >= 28: batch_size = 10 # Endgame cash liquidation
    ...
    # B) Plant Tasks (Urgent Water, Regular Water, Harvest)
        ...
        for u_idx, upos in enumerate(all_units):
            ...
            if upos in SHED_TILES:
                ...
                if empty_structures and (shed.get("COW", 0) > 0 or shed.get("SHEEP", 0) > 0) and not (inv.get("COW") or inv.get("SHEEP")):
                    ...
                    continue
                # 3. Dedicated livestock keepers (u_idx <= 2) always maintain wheat
            ...
            for u_idx, upos in enumerate(all_units):
                ...
                for t in tasks:
                    ...
                    if u_idx in [0, 1] and t["type"] in ["FEED", "CARE", "HARVEST_ANIMAL", "COLLECT_FERTILIZER", "BUILD_PASTURE", "PLACE_ANIMAL"]:
                        score += 300
                    # Field Workers (Units 2+) prefer crops/weeds
