...
class DailyPlanner:
    ...
    def plan_tasks(self):
        ...
        for crop_name in crop_actions.keys():
            ...
            shed_inventory = self.state.shed.get(crop_name, 0)
            ...
            seed_inventory = self.state.seeds.get(crop_name, 0)
            future_production = (planted_count + seed_inventory) * achievable_yield
            
            pipeline = market_excess + shed_inventory + future_production
            
            # --- Metrics Tracking ---
            if "pipeline_values" not in metrics:
                metrics["pipeline_values"] = {c: [] for c in ["WHEAT", "CARROT", "TOMATO", "STRAWBERRY", "MELON"]}
            metrics["pipeline_values"][crop_name].append(pipeline)
            
            if pipeline > TARGET_PIPELINE:
                if "c1_rejections" not in metrics:
                    metrics["c1_rejections"] = 0
                metrics["c1_rejections"] += 1
                continue # Crop rejected by C1 pacing constraint
                
            # Calculate EV using current market price unchanged
            expected_revenue = 0
...
def agent(obs):
    ...
    with open('debug_agent.txt', 'a') as f:
        ...
        try:
            ...
            f.write(f'Tasks planned: {len(tasks)}, seeds: {state.seeds}, money: {state.money}\n')
            for t in tasks:
                f.write(f'Task: {t.action_type} {t.priority} {t.kwargs}\n')
            ...
            f.write(f'Actions returned: {actions}, metrics_id: {id(MetricsTracker.get())}, metrics_plants: {MetricsTracker.get()["crops"]["MELON"]["planted"]}\n')
            return actions
