...
class TaskAllocator:
    ...
    def allocate(self):
        ...
        available_seeds = sum(self.state.seeds.values())
        uninstantiated_plant_tasks = max(0, min(empty_unlocked, available_seeds) - queued_plant_tasks)
        ...
        # V016-D: Strict dynamic labor scaling
        import math
        needed_workers = math.ceil((total_tasks / max(1.0, float(remaining_turns))) * 1.5)
        needed_hands = max(0, needed_workers - 1)
        
        if self.state.hires_today < needed_hands and remaining_turns > 3:
