"""
Agent V022-B: Dynamic Labor & Industrial Telemetry (v5 - Paced Multi-Hour Market Pipeline)
-----------------------------------------------------------------------------------------
Major Architectural Breakthrough:
1. Paced Market Pipeline:
   - Hour 0: WORKFORCE HIRING FIRST (up to 12 HIRE orders, 100% capacity) + Harvest Selling.
   - Hour 1: Capital Allocation (BUY_LAND, BUY_ANIMAL, BUY_SEED, BUY_PRODUCT feed).
   - Hour 2-23: Real-time Sales (Monetize newly dropped Milk, Wool, Fertilizer immediately).
2. Labor Capacity matches Unlocked Quadrants:
   - 1 Quad (Days 0-5): 2 hands ($2/day) -> perfectly covers 4 Sheep + 7 Melons.
   - 2 Quads (Days 6-9): 6 hands ($20/day) -> covers 4 Sheep + 6 Cows + Wheat.
   - 3 Quads (Days 10-13): 10 hands ($143/day) -> covers 15 Animals + 30 Strawberries.
   - 4 Quads (Days 14-30): 12 hands ($376/day) -> 312 daily actions, executing 100% animal care, 45+ strawberry watering & harvesting.
3. Compact Livestock Sector: Rows 0..4, Cols 2..7 for instant 1-3 step transit.
4. Urgent Watering Priority: Unwatered crops get weight 1600.
"""
...
WEIGHT_URGENT_WATER = 1600
WEIGHT_CLEAR_WEED = 1100
WEIGHT_FEED_ANIMAL = 1000
WEIGHT_CARE_ANIMAL = 950
WEIGHT_HARVEST_LIVESTOCK = 900
WEIGHT_WATER_PLANT = 850
WEIGHT_COLLECT_FERTILIZER = 800
WEIGHT_HARVEST_CROP = 750
WEIGHT_BUILD_STRUCTURE = 700
WEIGHT_PLACE_ANIMAL = 650
WEIGHT_PLANT_SEED = 600
...
def agent(obs):
    ...
    # -------------------------------------------------------------------------
    # 2. PACED MULTI-HOUR MARKET PIPELINE
    # -------------------------------------------------------------------------
    market_orders = []
    ...
    if day == 0 and hour == 0:
        ...
        # HOUR 0: LABOR HIRING FIRST & PRODUCE SELLING
        if hour == 0:
            # 1. Target labor based on unlocked land
            if day < 6:
                target_hands = 2
            elif num_quads >= 4:
            ...
            # 2. Sell existing shed produce (fills remaining order slots)
            for prod in ["FERTILIZER", "MILK", "WOOL", "MELON", "STRAWBERRY", "CARROT", "EGG"]:
                if len(market_orders) >= 10: break
                p_count = shed.get(prod, 0)
                if p_count > 0:
                    market_orders.append(["SELL", prod, min(p_count, 10)])

        # HOUR 1: CAPITAL ALLOCATION (Land, Animals, Seeds, Feed)
        elif hour == 1:
        ...
        # HOURS 2-23: REAL-TIME CONTINUOUS SELLING
        else:
            for prod in ["FERTILIZER", "MILK", "WOOL", "MELON", "STRAWBERRY", "CARROT", "EGG"]:
                p_count = shed.get(prod, 0)
                if p_count > 0 and len(market_orders) < 10:
                    market_orders.append(["SELL", prod, min(p_count, 10)])
    ...
    # Days 1-30: Dynamic Hungarian Task Assignment
    if not unit_actions:
        ...
        for u_idx, upos in enumerate(all_units):
            ...
            if upos in SHED_TILES:
                ...
                if empty_structures and (shed.get("COW", 0) > 0 or shed.get("SHEEP", 0) > 0) and not (inv.get("COW") or inv.get("SHEEP")):
                    ...
                    continue
                # 3. Pick up wheat if unit has low wheat and animals exist
