"""
Candidate V026: Leaderboard Meta Enhancements with Guaranteed Animal Feed Security
-----------------------------------------------------------------------------------
Upgrades applied to V025-A:
1. Safe Early Strawberry Expansion (Days 7-20): Enforces a $500 cash reserve so animals never starve.
2. Active Late-Game Strawberry Succession (Days 21-27): Identifies strawberry bushes aged >= 15 with 0 yield, issues DIG tasks, and infills with rapid 2-day Wheat.
3. Rapid Wheat Succession Cycle (Days 21-27): Plants 20-30 Wheat on cleared tiles for 2-3 extra harvest cycles before Day 30.
4. Complete Market Liquidation (Day 29).
"""
...
def agent(obs):
    ...
    if day == 0 and hour == 0:
        ...
        if hour in [1, 6, 12, 18]:
            ...
            # Safe Strawberry Seed Expansion (Days 7-20)
            if day >= 7 and day <= 20 and money >= 500:
            ...
            # Endgame Wheat Infill & Succession (Days 21-27)
            if day >= 21 and day <= 27 and money >= 150:
                wheat_seeds = seeds.get("WHEAT", 0)
                if (num_wheat_plants + wheat_seeds) < 40 and len(empty_unlocked_tiles) > 2:
                    market_orders.append(["BUY_SEED", "WHEAT", min(10, 40 - (num_wheat_plants + wheat_seeds))])
    ...
    for (px, py, crop, is_wat, y_u, age, max_life, unwat) in plants_on_board:
        # Active Late-Game Strawberry Succession (Days 21-27)
        if crop == "STRAWBERRY" and day >= 21 and day <= 27:
            if age >= 16 and y_u == 0:
                tasks.append({"type": "DIG", "pos": (px, py), "weight": WEIGHT_SUCCESSION_DIG})
                continue
    ...
    if day == 0:
        ...
        for ep in empty_unlocked_tiles:
            ...
            if day >= 21 and avail_wheat_seeds > 0:
                # Late-game succession priority: Plant fast wheat
                tasks.append({"type": "PLANT", "pos": ep, "weight": WEIGHT_PLANT_SEED, "crop": "WHEAT"})
                avail_wheat_seeds -= 1
            elif avail_straw_seeds > 0 and day <= 18:
                tasks.append({"type": "PLANT", "pos": ep, "weight": WEIGHT_PLANT_SEED, "crop": "STRAWBERRY"})
                avail_straw_seeds -= 1
            elif avail_wheat_seeds > 0 and day <= 27:
                tasks.append({"type": "PLANT", "pos": ep, "weight": WEIGHT_PLANT_SEED, "crop": "WHEAT"})
                avail_wheat_seeds -= 1
    ...
    for u_idx, u_pos in enumerate(all_units):
        ...
        if u_pos in SHED_TILES:
            carried_sellable = sum(u_inv.get(p, 0) for p in ["MILK", "WOOL", "STRAWBERRY", "MELON", "FERTILIZER", "CARROT", "WHEAT", "EGG"])
