        ...
        if field_tasks and units:
            ...
            for i, (ux, uy) in enumerate(units):
                for j, target in enumerate(field_tasks):
                    ...
                    # The V006-C TSP Fix:
                    # Priority previously dominated distance (e.g., dist*10 vs priority 300).
                    # This caused workers to cross the map to chase a slightly higher priority crop.
                    # By making distance penalty non-linear (squared), workers heavily prefer local clusters.
                    
                    if target.priority >= 1000:
                        # Massive negative cost ensures this task is matched to SOME worker
                        cost = -1000000 + (dist ** 2) * 10
                    else:
                        # Soft optimization: penalize long distance exponentially
