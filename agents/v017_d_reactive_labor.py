...
class TaskAllocator:
    ...
    def allocate(self):
        ...
        # V017-D: Reactive Idle-Aware Labor
        # Instead of calculating predicted labor tasks, look at the actual idle turns 
        # from the previous day. If idle turns are low, and we have unwatered plants or unplanted seeds, hire.
        idle_turns = self.metrics["workers"].get("idle_turns_yesterday", 0)
        unwatered = 0
        empty_unlocked = 0
        farm_tiles = self.state.my_farm.get("tiles", [])
        for y in range(self.state.board_size):
        ...
        # If we have unwatered plants or need to plant, and we didn't waste time idling yesterday, we hire.
        if (unwatered > 0 or uninstantiated_plant_tasks > 0) and idle_turns < 10:
...
def agent(obs):
    try:
        ...
        # V017-D: Save idle turns for tomorrow's hiring decisions
        if state.hour == 23:
            MetricsTracker.get()["workers"]["idle_turns_yesterday"] = MetricsTracker.get()["workers"].get("idle_turns", 0)
            MetricsTracker.get()["workers"]["idle_turns"] = 0
