"""
Candidate V023-M: Market-Aware Succession
-----------------------------------------
Features:
1. V023-G Capital allocation core.
2. Market-Aware Dynamic Crop Selection:
   Evaluates Day-30 liquidation value of Wheat vs Carrot vs Strawberry vs Melon based on real-time market prices.
3. Selects optimal replacement crop per tile on Days 21-27.
4. Paced seed purchasing and agile labor scheduling.
"""
...
WEIGHT_DIG_EXHAUSTED = 860
WEIGHT_PLANT_SEED = 840
...
def evaluate_best_replacement_crop(current_day, market_prices):
    days_left = 30 - current_day
    if days_left < 3:
        return None, 0
        
    p_wheat = market_prices.get("WHEAT", 50)
    p_carrot = market_prices.get("CARROT", 60)
    
    wheat_cycles = (days_left - 1) // 2
    net_wheat = max(0, wheat_cycles * 2 * p_wheat - 10)
    
    carrot_cycles = (days_left - 1) // 2
    net_carrot = max(0, carrot_cycles * 2 * p_carrot - 20)
    
    if net_carrot > net_wheat:
        return "CARROT", net_carrot
    else:
        return "WHEAT", net_wheat
...
def agent(obs):
    ...
    p_straw = market_prices.get("STRAWBERRY", 75)
    best_crop, best_crop_val = evaluate_best_replacement_crop(day, market_prices)
        ...
        for c in range(10):
            ...
            if t == "LOCKED":
                ...
                if kind == "WEED":
                    ...
                    elif crop == "CARROT": num_carrot_plants += 1
    ...
    if day == 0 and hour == 0:
        ...
        if hour == 0:
            ...
            # Market-aware seed buying on Days 21-27
            if day >= 21 and day <= 27 and money >= 150 and best_crop:
                target_seeds = seeds.get(best_crop, 0)
                active_count = num_wheat_plants if best_crop == "WHEAT" else num_carrot_plants
                if (active_count + target_seeds) < 48:
                    buy_amt = min(10, 48 - (active_count + target_seeds))
                    market_orders.append(["BUY_SEED", best_crop, buy_amt])
            elif day >= 6 and day < 21:
    ...
    for (px, py, crop, is_wat, y_u, age, max_life, unwat) in plants_on_board:
        ...
        rem_straw_days = max(0, min(days_left, 16 - age))
        exp_straw_val = (y_u + rem_straw_days * 0.8) * p_straw
        is_replace_straw = (crop == "STRAWBERRY" and day >= 21 and day <= 27 and best_crop_val > exp_straw_val + 15)
        
        if is_ripe and y_u > 0:
            tasks.append({"type": "HARVEST_CROP", "pos": (px, py), "weight": WEIGHT_HARVEST_CROP, "crop": crop})
        elif is_replace_straw:
            tasks.append({"type": "DIG", "pos": (px, py), "weight": WEIGHT_DIG_EXHAUSTED})
        elif not is_wat and not is_replace_straw:
    ...
    avail_carrot = seeds.get("CARROT", 0)
    avail_melon = seeds.get("MELON", 0)
    
    if (avail_straw > 0 or avail_wheat > 0 or avail_carrot > 0 or avail_melon > 0) and empty_unlocked_tiles:
        ...
        for it in interior_tiles[:35]:
            ...
            elif day >= 21 and day <= 27 and best_crop and seeds.get(best_crop, 0) > 0: chosen_crop = best_crop
            elif avail_wheat > 0 and day <= 27: chosen_crop = "WHEAT"
            elif avail_carrot > 0 and day <= 27: chosen_crop = "CARROT"
