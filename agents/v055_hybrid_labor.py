...
def _dynamic_auxiliary(action, obs, state, step):
    ...
    # 1. Dynamic Market Injections
    # If we have excess money, hire up to 10 hands
    if money > 2000 and day > 5 and hires_today < 10 and len(market) < 10:
        if not any(o for o in market if o and o[0] == "HIRE"):
            market.insert(0, ["HIRE"])
            money -= 100
            
    # Buy 4th quadrant if very rich
    if money > 4500 and "SE" not in unlocked and len(market) < 10:
        if not any(o for o in market if o and o[0] == "BUY_LAND"):
            market.insert(0, ["BUY_LAND"])
            money -= 4000
            
    # Buy Strawberry seeds if SE unlocked
    private = _get(obs, "private", {}) or {}
    seeds = _get(private, "seeds", {}) or {}
    straw_seeds = int(_get(seeds, "STRAWBERRY", 0))
    
    if "SE" in unlocked and money > 500 and straw_seeds < 10 and len(market) < 10:
        if not any(o for o in market if o and o[0] == "BUY_SEED" and len(o) > 1 and o[1] == "STRAWBERRY"):
            market.insert(0, ["BUY_SEED", "STRAWBERRY", 10])
            
    action["market"] = market[:10]
    
    # 2. Auxiliary Hand Assignment
    expected_core = len(action.get("hands") or [])
    actual_hands = _get(farm, "hands", []) or []
    total_hands = len(actual_hands)
    
    hands_actions = list(action.get("hands") or [])
    if total_hands <= expected_core:
        if len(hands_actions) < total_hands:
            hands_actions.extend([["PASS"] for _ in range(total_hands - len(hands_actions))])
        action["hands"] = [list(order or ["PASS"]) for order in hands_actions[:total_hands]]
        return action
        
    tiles = _get(farm, "tiles", []) or []
    weeds, unwatered, ripe, empty_se = [], [], [], []
    
    for y in range(len(tiles)):
        for x in range(len(tiles[y])):
            tile = tiles[y][x]
            if tile is None:
                if x >= 5 and y >= 5 and "SE" in unlocked:
                    empty_se.append((x, y))
    ...
    for i in range(expected_core, total_hands):
        pos = actual_hands[i]
        hx, hy = pos[0], pos[1]
        
        if len(ripe) > 0:
            target = min(ripe, key=lambda t: abs(t[0]-hx) + abs(t[1]-hy))
            if target[0] == hx and target[1] == hy:
                hands_actions.append(["HARVEST"])
            else:
                hands_actions.append(_step_towards(hx, hy, target[0], target[1]))
            ripe.remove(target)
        elif len(unwatered) > 0:
            target = min(unwatered, key=lambda t: abs(t[0]-hx) + abs(t[1]-hy))
            if target[0] == hx and target[1] == hy:
                hands_actions.append(["WATER"])
            else:
                hands_actions.append(_step_towards(hx, hy, target[0], target[1]))
            unwatered.remove(target)
        elif straw_seeds > 0 and len(empty_se) > 0:
            target = min(empty_se, key=lambda t: abs(t[0]-hx) + abs(t[1]-hy))
            if target[0] == hx and target[1] == hy:
                hands_actions.append(["PLANT", "STRAWBERRY"])
                straw_seeds -= 1
            else:
                hands_actions.append(_step_towards(hx, hy, target[0], target[1]))
            empty_se.remove(target)
        elif len(weeds) > 0:
            target = min(weeds, key=lambda t: abs(t[0]-hx) + abs(t[1]-hy))
            if target[0] == hx and target[1] == hy:
...
def agent(obs, configuration=None):
    try:
        ...
        return _dynamic_auxiliary(action, obs, state, step)
    except Exception:
        farm = _farm(obs, _seat(obs))
        return {
            "farmer": ["PASS"],
