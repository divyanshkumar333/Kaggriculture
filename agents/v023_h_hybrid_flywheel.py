"""
Candidate V023-H: Hybrid Replay-Derived Opening + Dynamic Frontier Optimizer
-----------------------------------------------------------------------------
Combines:
1. Replay-Exact Day 0-5 Opening (4 Sheep + 7 Melons + 5 Wheat).
2. Dynamic Capital Frontier Allocator on Days 6-29:
   - Evaluates remaining season days (D_rem = 30 - day)
   - Evaluates marginal revenue per resource (Cow, Sheep, Strawberry, Wheat)
   - Enforces 3-Quadrant compact spatial efficiency
   - Late-game wheat infill on Days 22-26
   - Paced monetization at town demand sinks.
"""
...
def agent(obs):
    ...
    if day == 0 and hour == 0:
        ...
        # HOUR 1: DYNAMIC CAPITAL FRONTIER OPTIMIZER
        elif hour == 1:
            ...
            # 2. Dynamic Marginal ROI Ranking
            # Remaining days in season
            d_rem = max(1, 30 - day)
            
            # Cow ROI: (d_rem - 2) * 450 / 1600
            cow_roi = max(0, (d_rem - 2) * 450) / 1600.0 if d_rem > 2 else 0
            # Strawberry ROI: min(d_rem - 5, 8) * 120 / 25
            straw_roi = max(0, min(d_rem - 5, 8) * 120) / 25.0 if d_rem > 5 else 0
            # Wheat ROI: max(0, d_rem - 2) * 60 / 10
            wheat_roi = max(0, (d_rem - 2) * 60) / 10.0 if d_rem > 2 else 0
            
            # If Cow ROI is high (Days 6-12), prioritize Cows
            if day >= 6 and day <= 14 and num_animals < 15:
            ...
            # If Strawberry ROI is active (Days 9-20), scale Strawberries
            if straw_roi > 2.0 and day >= 9 and day <= 20 and money >= 300:
            ...
            # Late-game Wheat Infill (Days 22-26)
