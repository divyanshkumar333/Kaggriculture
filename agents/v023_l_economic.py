"""
Candidate V023-L: Economic Succession
-------------------------------------
Features:
1. V023-G Capital allocation core.
2. Tile-Level Economic Replacement Model:
   Replaces a strawberry bush iff E[Wheat Profit for remaining days] > E[Remaining Strawberry Profit] + Transition Cost.
3. Dynamically prices remaining strawberry yield vs fast 2-day wheat cycles.
4. Optimal labor dispatch for DIG and PLANT.
"""
...
def should_replace_strawberry(age, yield_units, current_day, straw_price, wheat_price):
    if current_day > 27:
        return False
    days_left = 30 - current_day
    rem_prod_days = max(0, min(days_left, 16 - age))
    
    # Expected strawberry remaining revenue
    exp_straw_rev = (yield_units + rem_prod_days * 0.8) * straw_price
    
    # Expected wheat revenue if replanted on this tile
    wheat_cycles = (days_left - 1) // 2
    exp_wheat_rev = wheat_cycles * 2 * wheat_price - 10 # 10 seed cost
    
    # Replace if wheat net expected profit > remaining strawberry value + transition premium
    return exp_wheat_rev > exp_straw_rev + 15
...
def agent(obs):
    ...
    if day == 0 and hour == 0:
        ...
        if hour == 0:
            ...
            if day >= 21 and day <= 27 and money >= 100:
                wheat_seeds = seeds.get("WHEAT", 0)
                if (num_wheat_plants + wheat_seeds) < 48:
                    buy_wheat = min(10, 48 - (num_wheat_plants + wheat_seeds))
    ...
    for (px, py, crop, is_wat, y_u, age, max_life, unwat) in plants_on_board:
        ...
        is_replace_straw = (crop == "STRAWBERRY" and should_replace_strawberry(age, y_u, day, p_straw, p_wheat))
