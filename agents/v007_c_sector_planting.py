...
_METRICS = collections.defaultdict(lambda: {
    ...
    "spatial": {"fragmentation_sum": 0, "isolated_clusters": 0, "samples": 0},
    "_prev_plants": {}
})
    ...
    def plan_tasks(self):
        ...
        while total_seeds_to_plant > 0 and empty_tiles:
            ...
            for ex, ey in empty_tiles:
                ...
                if existing_crops:
                    same_crops = [(cx, cy) for cx, cy, cname in existing_crops if cname == seed_to_plant]
                    if same_crops:
                        min_dist_to_crop = min(abs(cx - ex) + abs(cy - ey) for cx, cy in same_crops)
                    else:
