"""
Candidate V025-D: Generalized Marginal-ROI Dynamic Capital Allocator
--------------------------------------------------------------------
Features:
1. Hourly dynamic portfolio valuation based on Marginal Lifetime ROI:
   - Strawberry seed: (Days_left - 4) * $54 / $20 (High ROI on D1-D15)
   - Cow: (Days_left - 1) * $120 / $1,500 (High absolute cash flow on D4-D12)
   - Land: (25 tiles * Net Tile Margin - Cost) / Cost
   - Labor: Opportunity value of daily care/watering vs fib(n) hire cost
2. Zero artificial day locks: Assets compete purely on marginal expected contribution.
"""
    ...
    if day == 0 and hour == 0:
        ...
        if hour == 0:
            if day < 4:
                ...
                target_hands = 5 if money >= 50 else 3
            elif num_quads >= 3:
                target_hands = 12 if day >= 10 and money >= 300 else 8
            elif num_quads >= 2:
                target_hands = 7 if money >= 80 else 4
            else:
                target_hands = 4
        ...
        # DYNAMIC MARGINAL ROI EVALUATION INTRA-DAY
        if hour in [1, 6, 12, 18]:
            ...
            # ROI Calculations:
            # 1. Strawberry Seed ROI:
            straw_active = max(0, min(16, days_left - 4))
            straw_roi = (straw_active * 54.0 - 20.0) / 20.0 if straw_active > 0 else -1.0
            
            # 2. Cow ROI:
            cow_active = max(0, days_left - 1)
            cow_roi = (cow_active * 120.0 - 1500.0) / 1500.0 if cow_active > 0 else -1.0
            
            # Strawberry seed purchase (Highest marginal % ROI on D1-D18)
            straw_seeds = seeds.get("STRAWBERRY", 0)
            max_straw = 15 if num_quads == 1 else (30 if num_quads == 2 else 42)
            if straw_roi > 1.0 and (num_strawberries + straw_seeds) < max_straw and len(empty_unlocked_tiles) > 2 and money >= 100:
                buy_straw = min(8, max_straw - (num_strawberries + straw_seeds))
            ...
            # Cow Purchase (Highest absolute cash flow)
            cows_in_shed = shed.get("COW", 0)
            if cow_roi > 0.1 and (num_cows + cows_in_shed) < 11:
