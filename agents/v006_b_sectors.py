...
class ActionExecutor:
    ...
    def execute(self, tasks, assignments):
        ...
        if field_tasks and units:
            ...
            sectors = []
            for q in unlocked:
                if q == "NW": sectors.append((0, 4, 0, 4))
                elif q == "NE": sectors.append((5, 9, 0, 4))
                elif q == "SW": sectors.append((0, 4, 5, 9))
                elif q == "SE": sectors.append((5, 9, 5, 9))
            if not sectors:
                sectors = [(0, 4, 0, 4)]
            ...
            for i, (ux, uy) in enumerate(units):
                ...
                for j, target in enumerate(field_tasks):
                    ...
                    # Sector constraint
                    out_of_sector_penalty = 0
                    if not (sx1 <= tx <= sx2 and sy1 <= ty <= sy2):
                        out_of_sector_penalty = 10000
                    ...
                    if target.priority >= 1000:
                        ...
                        cost = -1000000 + dist * 10 + out_of_sector_penalty
                    else:
