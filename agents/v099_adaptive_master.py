"""
Candidate V099: Adaptive Master (Cycle 1)
---------------------------------------------------
Features:
1. Base execution: Hungarian task assignment and strict V025-A opening.
2. Opponent Model: Computes likelihood of Cow Rush vs Berry Flywheel at Day 6.
3. Policy Switch: Evaluates expected value dynamically and pivots capital allocation if opponent is exploiting the milk market.
"""
...
def agent(obs):
    ...
    # -------------------------------------------------------------
    # ADAPTIVE MASTER POLICY: BAYESIAN OPPONENT MODELING
    # -------------------------------------------------------------
    # At day 6 (turn 144), observe public features and branch.
    active_policy = "V025-A"  # Default opening
    if day >= 6:
        opp_idx = 1 - obs.get("player", 0)
        opp_farm = obs.get("farms", {})[opp_idx] if "farms" in obs else {}
        ...
        # Bayesian Posterior simplification
        if opp_cows >= 3:
            # High probability of COW_RUSH. 
            # EV dictates BERRY_FLYWHEEL to prevent mutual market crash.
            active_policy = "BERRY_FLYWHEEL"
        else:
            # High probability of Crop Flywheel. 
            # EV dictates V097 logic (cow domination, late pivot only on crash)
            active_policy = "V097"
    ...
    if day == 0 and hour == 0:
        ...
        if hour == 0:
            if active_policy == "BERRY_FLYWHEEL":
                target_hands = 12 if money >= 150 else 8
            else:
                if day < 4:
                    target_hands = 2
                elif day < 6:
        ...
        if hour in [1, 6, 12, 18]:
            ...
            if active_policy == "BERRY_FLYWHEEL":
                # Hard Pivot: Stop cows, buy strawberry seeds heavily
                if money >= 20 and len(empty_unlocked_tiles) > 2:
                    buy_straw = min(10, max(1, int(money / 20)))
                    market_orders.append(["BUY_SEED", "STRAWBERRY", buy_straw])
            elif active_policy == "V097":
                # Late Pivot: Pivot to berries only if milk crashes
                if market_prices.get("MILK", 100) <= 50 and money >= 40:
                    market_orders.append(["BUY_SEED", "STRAWBERRY", min(10, int(money / 20))])
                else:
                    # Normal cow scaling
