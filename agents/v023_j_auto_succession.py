"""
Candidate V023-J: Automatic Strawberry -> Wheat Succession
----------------------------------------------------------
Features:
1. V023-G Capital allocation core.
2. Days 21-27: Automatic detection of exhausted strawberry bushes (age >= 12).
3. Ceases watering exhausted strawberry bushes.
4. Generates DIG tasks on exhausted strawberry tiles and PLANT WHEAT tasks on freed tiles.
5. Paced Wheat seed purchases (batches of 10 up to 48 plants) and rapid 2-day harvest liquidation.
"""
...
def agent(obs):
    ...
    if day == 0 and hour == 0:
        ...
        if hour == 0:
            ...
            # Strawberry planting up to Day 20
            ...
            # Endgame Wheat Succession (Days 21-27)
            if day >= 21 and day <= 27 and money >= 100:
                wheat_seeds = seeds.get("WHEAT", 0)
                # Allow up to 48 wheat plants to fully populate freed strawberry tiles
    ...
    for (px, py, crop, is_wat, y_u, age, max_life, unwat) in plants_on_board:
        ...
        # Check if strawberry is exhausted
        is_exhausted_straw = (crop == "STRAWBERRY" and day >= 21 and age >= 12)
        ...
        if is_ripe and y_u > 0:
            ...
            # Clear exhausted strawberry tile so wheat can be planted
    ...
    if (avail_straw > 0 or avail_wheat > 0 or avail_melon > 0) and empty_unlocked_tiles:
        ...
        for it in interior_tiles[:35]:
            ...
            elif avail_straw > 0 and day < 21: chosen_crop = "STRAWBERRY"
