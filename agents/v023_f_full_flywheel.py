"""
Candidate V023-F: Full Flywheel (Cows-First + Strawberry Scaling + Late Succession)
-----------------------------------------------------------------------------------
1. Day 6: Wool cash -> Q2 Unlock + Cow expansion.
2. Days 7-8: Reinvest milk/fertilizer cash into cows (reaching 6-8 cows).
3. Days 9-11: Ramped 45-Strawberry planting across Q2/Q3.
4. Days 12-21: Full Industrial Engine (4 Sheep, 8-11 Cows, 45 Strawberries).
5. Days 22-26: Replant decaying strawberry tiles with 2-day fast Wheat & Carrots.
6. Days 27-29: Full endgame liquidation.
"""
...
def agent(obs):
    ...
    if day == 0 and hour == 0:
        ...
        # HOUR 1: CAPITAL DEPLOYMENT (Land, Cows-First, Seeds, Feed)
        elif hour == 1:
            ...
            if "NE" not in unlocked_quads and money >= 1400 and day >= 6:
                ...
                market_orders.append(["BUY_LAND"])
            elif "SE" not in unlocked_quads and money >= 7000 and day >= 13:
                market_orders.append(["BUY_LAND"])
                
            # 2. PRIORITY COW ACCELERATION
            ...
            # 3. Strawberry Seed Purchases (Days 9-20)
            ...
            # 4. Late-Season Crop Succession (Days 22-26)
            if day >= 22 and day <= 26 and money >= 200:
                wheat_seeds = seeds.get("WHEAT", 0)
                if (num_wheat_plants + wheat_seeds) < 25 and len(empty_unlocked_tiles) > 2:
                    market_orders.append(["BUY_SEED", "WHEAT", min(10, 25 - (num_wheat_plants + wheat_seeds))])
            elif day >= 6 and day < 22:
                wheat_seeds = seeds.get("WHEAT", 0)
                if (num_wheat_plants + wheat_seeds) < 10 and money >= 100:
                    market_orders.append(["BUY_SEED", "WHEAT", 4])

        # HOURS 2-23: PACED DYNAMIC SELLING
        else:
    ...
    for (px, py, crop, is_wat, y_u, age, max_life, unwat) in plants_on_board:
        ...
        # Clear finished strawberries on Day 23+
        if crop == "STRAWBERRY" and day >= 23 and y_u == 0 and age >= 13:
            tasks.append({"type": "DIG", "pos": (px, py), "weight": WEIGHT_CLEAR_WEED})
        elif is_ripe and y_u > 0:
    ...
    if (avail_straw > 0 or avail_wheat > 0 or avail_melon > 0) and empty_unlocked_tiles:
        ...
        for it in interior_tiles[:25]:
            ...
            elif avail_straw > 0 and day >= 8 and day < 22: chosen_crop = "STRAWBERRY"
            elif avail_wheat > 0: chosen_crop = "WHEAT"
            tasks.append({"type": "PLANT", "pos": it, "weight": WEIGHT_PLANT_SEED, "crop": chosen_crop})

    # 4. ACTION SELECTION & HUNGARIAN DISPATCH
    unit_actions = []
    
    # DAY 0 SCRIPTED OPENING
    if day == 0:
