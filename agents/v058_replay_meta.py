...
def agent(obs):
    ...
    # -------------------------------------------------------------------------
    if day == 0 and hour == 0:
        # Replay Meta Opening: 2 Cows + 6 Melons + 5 Strawberries + 6 Wheat + 5 Hires
        ...
        # Note: 4 Cows would be great, but 2 Cows leaves capital for early strawberries
        market_orders.append(["BUY_PRODUCT", "WHEAT", 4])
        ...
        if hour in [1, 6, 12, 18]:
            ...
            # Calibrated Cow Capacity (Optimal: 4 Cows on Day 0, expand to 8 later)
            cows_in_shed = shed.get("COW", 0)
            target_cows = 4 if day < 5 else 8
            if day <= 15 and (num_cows + cows_in_shed) < target_cows:
                while money >= 500 and (num_cows + cows_in_shed) < target_cows and len(market_orders) < 8:
                    market_orders.append(["BUY_ANIMAL", "COW", 1])
                    money -= 400
                    cows_in_shed += 1
                    
            # Early Strawberry Powerhouse (Start Day 0, 40 capacity)
            straw_seeds = seeds.get("STRAWBERRY", 0)
            if day <= 20 and money >= 250:
