...
class DailyPlanner:
    ...
    def plan_tasks(self):
        ...
        while total_seeds_to_plant > 0 and empty_tiles:
            ...
            if best_tile:
                ...
                if remaining_days - 1 < crop_info.get("first_yield_day", 999):
                    ...
                    continue
                self.tasks.append(Task("PLANT", 30, best_tile, {"crop": seed_to_plant, "urgency_score": 0, "value_score": value, "pipeline": pipeline_qty}))
                simulated_seeds[seed_to_plant] -= 1
                total_seeds_to_plant -= 1
        ...
        for crop_name in crop_actions.keys():
            ...
            # --- V018-C Pipeline ---
            TARGET_PIPELINE = float(os.environ.get("V018_C_TARGET", 20.0))
            MECHANISM = os.environ.get("V018_C_MECH", "C1_HARD")
            
            market_inv = self.state.market.get("inventory", {}).get(crop_name, 10000)
            market_excess = max(0, market_inv - 10000)
            shed_inv = self.state.shed.get(crop_name, 0)
            
            planted_count = sum(1 for y in range(self.state.board_size) for x in range(self.state.board_size) 
                              if isinstance(farm_tiles[y][x], dict) and farm_tiles[y][x].get("kind") == "PLANT" and farm_tiles[y][x].get("crop") == crop_name)
            
            seed_count = self.state.seeds.get(crop_name, 0)
            future_yield = (planted_count + seed_count) * achievable_yield
            
            pipeline_qty = market_excess + shed_inv + future_yield
            
            expected_revenue = 0
            for i in range(achievable_yield):
                p = self.econ.get_price_at_inventory(crop_name, market_inv + i)
                expected_revenue += p
                
            if MECHANISM == "C1_HARD":
                if pipeline_qty > TARGET_PIPELINE:
                    expected_revenue = -999999
            elif MECHANISM == "C2_SOFT":
                multiplier = max(0.0, 1.0 - 0.1 * (pipeline_qty - TARGET_PIPELINE))
                expected_revenue *= multiplier
