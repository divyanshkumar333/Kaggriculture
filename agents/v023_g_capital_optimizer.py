"""
Candidate V023-G: Generalized Capital Allocation Optimizer
----------------------------------------------------------
Features:
1. Dynamic capital allocation ranking based on Remaining Lifetime ROI (Days left * Daily Margin / Cost).
2. Strict Operating Reserve protection for animal feed and daily labor.
3. Optimal 3-Quadrant Compact Density (avoids $7k Q4 trap, minimizing worker transit latency).
4. Dual-Phase Portfolio:
   - Days 6-9: High-Velocity Cow Acceleration (2 cows/day up to 11 cows).
   - Days 10-21: 40+ Strawberry Industrial Core.
   - Days 22-26: Endgame Wheat Infill on decaying strawberry tiles (2-day harvest burst).
5. Hungarian task assignment with role specialization.
6. Paced town demand sink monetization.
"""
...
def agent(obs):
    ...
    if day == 0 and hour == 0:
        ...
        # HOUR 1: CAPITAL DEPLOYMENT (Dynamic ROI Frontier)
        elif hour == 1:
            # 1. Feed Reserve First
            ...
            # 4. Strawberry Seed Expansion (Days 9-20)
            ...
            # 5. Endgame Wheat Infill on Freed Decaying Tiles (Days 22-26)
