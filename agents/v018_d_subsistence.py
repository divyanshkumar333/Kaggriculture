...
class DailyPlanner:
    ...
    def plan_tasks(self):
        ...
        EMPTY_LAND_TRIGGER = int(os.environ.get("V018_D_EMPTY_LAND_TRIGGER", 10))
        FALLBACK_MODE = os.environ.get("V018_D_FALLBACK_MODE", "WHEAT_ONLY")

        def eval_crops(crop_list):
            best_p = -999999
            best_c = None
        ...
        if best_profit_per_action <= 0 and len(empty_tiles) > EMPTY_LAND_TRIGGER:
            fallback_crops = ["WHEAT"] if FALLBACK_MODE == "WHEAT_ONLY" else ["WHEAT", "CARROT"]
            fb_crop, fb_profit = eval_crops(fallback_crops)
            if fb_profit > 0:
                best_crop = fb_crop
                best_profit_per_action = fb_profit
                
        target_crop = best_crop
        if target_crop and self.state.seeds.get(target_crop, 0) == 0 and self.state.money > self.strategy.config.cash_reserve + CROPS[target_crop]["seed"]:
