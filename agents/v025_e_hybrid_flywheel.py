"""
Candidate V025-E: Hybrid High-Velocity Flywheel + Generalized Optimizer
-----------------------------------------------------------------------
Features:
1. Days 0-10: High-Velocity Flywheel:
   - Early Strawberry Ramp: Plants 12-15 strawberries on Days 2-5.
   - Intra-Day Cow Velocity: Unlocks Cow purchasing from Day 4 onwards whenever cash >= $1,500.
   - Proactive Pasture Pre-Building: Ensures structures are ready before cows arrive.
   - Proactive Labor Scaling: 4 workers on D4, 7 on D5-7, 12 on D10+.
2. Days 11-30: Generalized Capital Optimization (V023-G Core):
   - 11 Cow Fleet Production
   - 42 Strawberry Industrial Core
   - Paced town market monetization
   - Safe endgame crop maintenance
"""
...
def agent(obs):
    ...
    if day == 0 and hour == 0:
        ...
        if hour in [1, 6, 12, 18]:
            ...
            # Quadrant Expansion (Q2 on D5, Q3 on D8)
            ...
            # Cow Acceleration (D4-D15, up to 11 cows)
            ...
            # Early Strawberry Ramp (Days 2-20)
            if day >= 2 and day <= 20 and money >= 150:
                straw_seeds = seeds.get("STRAWBERRY", 0)
                max_straw = 14 if num_quads == 1 else (28 if num_quads == 2 else 42)
                if (num_strawberries + straw_seeds) < max_straw and len(empty_unlocked_tiles) > 2:
                    buy_straw = min(6 if day < 6 else 10, max_straw - (num_strawberries + straw_seeds))
