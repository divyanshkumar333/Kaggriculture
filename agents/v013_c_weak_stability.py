...
class ActionExecutor:
    ...
    def execute(self, tasks, assignments):
        ...
        if field_tasks and units:
            ...
            import os as _os
            if not hasattr(self, "_PREV_ASSIGNMENTS"):
                self._PREV_ASSIGNMENTS = _collections.defaultdict(dict)
            _seed = _os.environ.get("KAGGRICULTURE_SEED", "unknown")
            ...
            for i, (ux, uy) in enumerate(units):
                for j, target in enumerate(field_tasks):
                    ...
                    # V013-C: Locality +80 (cross-quadrant penalty)
                    u_quad = (ux // 5, uy // 5)
                    t_quad = (tx // 5, ty // 5)
                    if u_quad != t_quad:
                        cost += 80

                    # V013-C: Weak stability -10 (only suppresses 1-step oscillation)
                    # ponytail: -10 < 1 distance step (10 pts), so a task 1 tile closer
                    # immediately wins; avoids stale-commitment from V012-B's -40.
                    if self._PREV_ASSIGNMENTS[_seed].get(i) == (tx, ty):
                        cost -= 10
            ...
            for i, j in zip(row_ind, col_ind):
                ...
                self._PREV_ASSIGNMENTS[_seed][i] = field_tasks[j].location
