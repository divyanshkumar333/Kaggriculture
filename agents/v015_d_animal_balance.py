                    ...
                    if tile.get("kind") == "PLANT":
                        ...
                        if "animal" in tile:
                            ...
                            if not tile.get("cared_today", True):
                                self.tasks.append(Task("CARE", 9, (x, y), {"urgency_score": 0, "value_score": 50}))
                            if tile.get("yield_units", 0) > 0:
                                self.tasks.append(Task("HARVEST", 20, (x, y), {"urgency_score": 0, "value_score": 150}))
                            if tile.get("fertilizer_available", False):
                                self.tasks.append(Task("COLLECT_FERTILIZER", 15, (x, y), {"urgency_score": 0, "value_score": 50}))
                        else:
                            expected_animal = "GOOSE" if tile.get("kind") == "COOP" else "COW"
                            for animal_type in ["GOOSE", "COW", "SHEEP"]:
                                if simulated_shed.get(animal_type, 0) > 0:
                                    if ANIMALS[animal_type]["structure"] == tile.get("kind"):
                                        self.tasks.append(Task("PLACE", 10, (x, y), {"item": animal_type, "urgency_score": 0, "value_score": 300}))
                    ...
                    for y in range(self.state.board_size):
                        for x in range(self.state.board_size):
                            if farm_tiles[y][x] is None and not any(t.location == (x,y) for t in self.tasks):
                                self.tasks.append(Task(f"BUILD_{structure_type}", 40, (x, y), {"urgency_score": 0, "value_score": 100}))
                                break
