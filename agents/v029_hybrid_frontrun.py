    ...
    if day == 0 and hour == 0:
        ...
        for prod in ["STRAWBERRY", "MILK", "WOOL", "MELON", "FERTILIZER", "CARROT", "WHEAT", "EGG"]:
            ...
            if p_count > 0:
                ...
                # Check for extreme situations where we shouldn't sell
                if cur_price <= 5 and (demand_now > 0 or demand_next > 0) and day < 28 and not urgency:
                    continue
                    
                if demand_now > 0 and not (urgency or day >= 27):
                    # Town is buying THIS turn. Wait for price to rise.
                    continue
                elif demand_next > 0 and cur_price > 5:
                    # Town is buying NEXT turn. Front-run the opponent! Sell as much as possible.
                    batch_size = p_count
                else:
